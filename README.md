...
- **GET `/api/search?q=...&k=3`** — семантический поиск топ-K документов по смысловой близости (косинусное сходство).
- **GET `/api/documents/count`** — количество документов в базе.
- **GET `/`** — простой веб-интерфейс с поисковой строкой и карточками результатов.
- **GET `/docs`** — Swagger UI.
